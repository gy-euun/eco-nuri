// Vercel Postgres + Prisma 사용 예시
// 실제 사용 시: lib/db.ts 파일을 이 코드로 교체하세요
...
const updateStats = async (buildingType: string) => {
  ...
  await prisma.stats.create({
    data: {
      ...
      totalCalculations: stats.totalCalculations,
      inquiriesByBuildingType: {
        ...stats.inquiriesByBuildingType,
        [buildingType]: (stats.inquiriesByBuildingType[buildingType] || 0) + 1
      },
      inquiriesByDate: {
        ...stats.inquiriesByDate,
        [today]: (stats.inquiriesByDate[today] || 0) + 1
      }
    }
  })
}
...
export const incrementCalculationCount = async () => {
  ...
  await prisma.stats.create({
    data: {
      ...
      inquiriesByBuildingType: stats.inquiriesByBuildingType,
      inquiriesByDate: stats.inquiriesByDate
    }
  })
}
